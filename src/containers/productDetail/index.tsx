...
import { ProductComment, ProductDes, ToasMess, Loading2, ProductShop, ProductImgPc, ProductContent } from '../../component/index';

interface ProductDetail {
  loading: boolean;
}
function ProductDetail({ loading }: ProductDetail) {
  ...
  return (
    <>
      {loading ? (
        ...
        <>
          {toastMess && <ToasMess content="Bạn Vừa Thêm 1 Sản Phẩm Vào Giỏ Hàng" />}
          <>
            <div className=" Hide-on-mobile mt-[120px]"></div>
            <div className="bg-[#f5f5f5] overflow-hidden App__Container_padding-Button mob:pt-[20px]">
              <div className="grid wide">
                <div className="row sm-gutter pb-[30px] bg-[white]">
                  ...
                  <div className="col c-12  mo-7 l-7 ">
                    <ProductContent item={data} amount={amount} NewOption={NewOption} setNewOption={setNewOption} onIncrease={handleIncrease} onReduced={handleReduced} onAddToCart={handleAddToCart} />
                  </div>
                </div>
              </div>
            </div>
